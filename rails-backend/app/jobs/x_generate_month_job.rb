class XGenerateMonthJob < ApplicationJob
  ...
  def perform(job_id, user_id, params)
    ...
    broadcast(job_id, type: "done",
              posts: created.map { |p| serialize_post(p) },
              event_announcements: event_announcements.map { |p| serialize_post(p) })
  rescue => e
  ...
  # NOTE: メソッド名を `serialize` にすると ActiveJob::Core#serialize（enqueue 時に
  # Sidekiq アダプタが呼ぶ公開メソッド）を private で上書きしてしまい、
  # perform_later が NoMethodError(private method serialize) で 500 になる。名前は必ず別にする。
