...
class PostJob < ApplicationJob
  ...
  def perform(job_id, payload)
    ...
    # オンクラスは「受講生サポート」タブからの投稿のみ許可（Playwright 起動前に判定）
    ...
    # ===== API 直叩き（ブラウザ不要）サイトは Playwright を使わず先に投稿する =====
    # オンクラスは curl/API 方式。Playwright（chromium）の起動失敗に巻き込まれて
    # 投稿ごと落ちるのを防ぐため、ブラウザ機構の外で先に処理する。
    api_only_sites = sites.select { |s| API_ONLY_SERVICES.key?(s.split(":").first) }
    sites = sites.reject { |s| API_ONLY_SERVICES.key?(s.split(":").first) }
    api_only_sites.each do |site_key|
      post_api_only_site(site_key, content, event_fields, image_path, item_id, posting_user, job_id)
    end

    # 以降はブラウザ必須サイトのみ。無ければ Playwright を起動しない（本節全体を if でガード）。
    if sites.any?
  ...
  # curl/API 方式でブラウザ(Playwright)を一切使わずに投稿できるサイト。
  # Playwright の起動に巻き込まれないよう、ブラウザ機構の外で先に投稿する。
  API_ONLY_SERVICES = {
    "オンクラス" => Posting::OnclassService
  }.freeze
  ...
  # ブラウザ不要のサイト（オンクラス等）を、Playwright を使わず API 直叩きで投稿する。
  def post_api_only_site(site_key, content, event_fields, image_path, item_id, posting_user, job_id)
    site_name, sub_type = site_key.split(":", 2)
    svc_class = API_ONLY_SERVICES[site_name]
    return unless svc_class
    ...
    broadcast(job_id, type: "status", site: site_name, status: "running")
    broadcast(job_id, type: "log", message: "[#{site_name}] 開始...（API直叩き）")

    log_fn = ->(msg) { broadcast(job_id, type: "log", message: msg) }
    svc_class.new.call(nil, content, ef, &log_fn)

    broadcast(job_id, type: "status", site: site_name, status: "success")
    update_connection_status(site_name, "connected")
    save_posting_history(item_id, site_name, "success", nil, true)
  rescue => e
    broadcast(job_id, type: "log", message: "[#{site_name}] ❌ エラー: #{e.message}")
    broadcast(job_id, type: "status", site: site_name, status: "error")
    update_connection_status(site_name, "error", e.message)
