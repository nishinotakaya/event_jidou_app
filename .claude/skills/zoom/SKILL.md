...
- **開催日変更 → Zoomタイトルの日付プレフィックス自動更新**（例: `4/25 テスト体験会`）
- **Zoom自動作成クリック時 → イベント詳細の開催日・開始時刻をZoomの開催日時に自動反映**
- 連動方向: イベント開催日 → Zoom開催日時（一方向）
...
**Zoom URL保存済みの場合 → 詳細ビュー（デフォルト）:**
- 🎥 Zoom 詳細: タイトル / URL（クリックで直接Zoom遷移）/ ミーティングID / パスコード
- 「✏️ 編集」ボタンで編集フォームに切り替え
- ヘッダーには「🔄 自動作成」「📥 読み込み」「🗑 クリア」ボタン

**Zoom URL未保存の場合 → 入力フォーム:**
- タイトル / URL / ミーティングID / パスコードの入力欄
- 「🔄 自動作成」「📥 読み込み」「💾 保存」ボタン

**共通操作:**
1. **読み込み**: 「📥 読み込み」→ ドロップダウンで保存済み設定一覧 → クリックで自動入力 → 詳細ビューに戻る
2. **保存**: 入力後「💾 保存」→ ラベル名入力 → DBに保存
3. **削除**: ドロップダウン内の「✕」→ 確認ダイアログ → DB削除
4. **自動作成後**: 自動的に詳細ビューに切り替わる
